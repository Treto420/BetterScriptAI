-- aiHornSync GE Extension  v1.3
-- Place at: mods/unpacked/aiHornSync/lua/ge/extensions/aiHornSync/main.lua
...
function M.onClientStartMission(levelPath)
    ...
    for i = 0, count - 1 do
        ...
        if veh then
            veh:queueLuaCommand("extensions.load('aiHornSync')")
        end
    end
end
...
local function getHonkPath(vehId)
    return "mods/unpacked/aiHornSync/honkdata/veh_" .. tostring(vehId) .. ".json"
end
...
function stopPlayback(vehId)
    ...
    local veh = be:getObjectByID(vehId)
    if veh then
        veh:queueLuaCommand("extensions.aiHornSync.triggerHonkOff()")
    end
    playbackState[vehId] = nil
        ...
        if ps.active then
            ...
            while ps.index <= #ps.honks do
                ...
                if ps.elapsed >= nextHonk.time then
                    local veh = be:getObjectByID(vehId)
                    if veh then
                        veh:queueLuaCommand("extensions.aiHornSync.triggerHonk()")
                        log("I", logTag, string.format(
                    ...
                    -- This correctly handles rapid overlapping presses.
